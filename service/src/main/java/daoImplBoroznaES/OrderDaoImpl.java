...
public class OrderDaoImpl implements OrderDAO {
    ...
    public Double getOrderStatus() {
        ...
        try {
            ...
            if (connection != null) {
                try (PreparedStatement preparedStatement = connection.prepareStatement(
                        "select sum(price)  as allprice from `order` where status=\"Completed\" "
                )) {
                    ResultSet resultSet = preparedStatement.executeQuery();
                    if (resultSet.next()) {
    ...
    @Override
    public Double getOrderStatusWaiting() {
        Connection connection = null;
        try {
            connection = connectionManager.getConnection();
            if (connection != null) {
                try (PreparedStatement preparedStatement = connection.prepareStatement(
                        "select sum(price)  as allprice from `order` where status=\"Waiting\" "
                )) {
                    ResultSet resultSet = preparedStatement.executeQuery();
                    if (resultSet.next()) {
                        Double price = resultSet.getDouble("allprice");
